import {  CommentCreatePayload, ICommentEntity, IProductSearchFilter, IProductImageEntity, IMatchEntity} from "../types";
import { IComment, IProduct, IProductImage } from "@Shared/types";
import { mapCommentEntity, mapImagesEntity, mapImageEntity} from "./services/mapping";
...
export const validateComment: CommentValidator = (comment) => {
    ...
    if (wrongField) {
        return `Field '${wrongField}' is absent`;
    }
...
//хелпер для сопоставления изображений и комментариев по productId
//сортировка изображений по product-id
...
// //ВОЗВРАТ PRODUCTS c matches
// export const enhanceProductsMatches = (
//     products: IProduct[],
//     matchesRows: IMatchEntity[]
// ): IProduct[] => {
//     const matchesByProductId = new Map < string, IMatch[]> ();

//     for (let matchEntity of matchesRows) {
//         const match = mapMatchEntity(matchEntity);
//         if (!matchesByProductId.has(match.productId)) {
//             matchesByProductId.set(match.productId, []);
//         }

//         const list = matchesByProductId.get(match.productId);
//         matchesByProductId.set(match.productId, [...list, match]);
//     }

//     for (let product of products) {
//         if (matchesByProductId.has(product.id)) {
//             product.matches = matchesByProductId.get(product.id);
//         }
//     }
